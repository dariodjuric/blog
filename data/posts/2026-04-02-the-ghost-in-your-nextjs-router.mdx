...
When Next.js introduced the App Router (`app/` directory), it didn't replace the Pages Router (`pages/` directory) overnight. Instead, it let both coexist in the same application so teams could migrate incrementally. But that raises a routing question: when a user clicks a link, which router should handle it?
...
Your user clicks a `<Link>` to `/activities`, which is a perfectly valid Pages Router page. The bloom filter incorrectly says "that might be an app route", so Next.js does a full page reload instead of a smooth client-side transition.
